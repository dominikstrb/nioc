...
def estimate_controls(
    x: jnp.array, env: Env, params: dict, maxiter: int = 100
) -> jnp.ndarray:
...
def draw_random_uniform_in_log_space(key, lo, hi, params_type):
    return jax.tree_util.tree_map(
        lambda key, lo, hi: random.uniform(
            key, minval=jnp.log10(lo), maxval=jnp.log10(hi)
        ),
        params_type(*random.split(key, len(params_type._fields))),
        lo,
        hi,
    )
...
def compute_mle(
    ...
    nll = lambda params: -ioc.loglikelihood(
        xs,
        ParamsType(**jax.tree_util.tree_map(lambda x: 10**x, params)),
        **likelihood_params,
    )
    ...
    init_params = [
        draw_random_uniform_in_log_space(key, *bounds, ParamsType)
        for key in random.split(subkey, restarts)
    ]
    # only use initial params of those parameters that we want to infer
    init_params = [
        {
            param_name: param_value
            for param_name, param_value in p._asdict().items()
    ...
    bounds = tuple(
        {param_name: getattr(bound, param_name) for param_name in params}
        for bound in bounds
    )

    # run inference
    result = run_with_restarts(
        optim, init_params=init_params, bounds=jax.tree_util.tree_map(jnp.log10, bounds)
    )

    # transform back into original parameter space
    result = jaxopt_base.OptStep(
        params=ParamsType(**jax.tree_util.tree_map(lambda x: 10**x, result.params)),
        state=result.state,
    )
